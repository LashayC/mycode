...
def main():
    ...
    # show the first 5 rows of our DF
    # DF has 5 rows and 25 columnnes (indexed by integer)
    print(movies.head())

    # Choose the first column "Title" as index (index = 0)
    movies_sheet1 = pd.read_excel(excel_file, sheet_name=0, index_col=0)
    # DF has 5 rows and 24 columnes (indexed by title)
    print(movies_sheet1.head())

    # grap the next 2 sheets as well
    movies_sheet2 = pd.read_excel(excel_file, sheet_name=1, index_col=0)
    # DF has 5 rows and 24 columns (indexed by title)
    print(movies_sheet2.head())

    movies_sheet3 = pd.read_excel(excel_file, sheet_name=2, index_col=0)
    # DF has 5 rows and 24 coumns (indexed by title)
